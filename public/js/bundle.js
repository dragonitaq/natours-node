...
var displayMap = function displayMap(locations) {
  ...
  var bounds = new mapboxgl.LngLatBounds(); // We loop through each location point in our tour to create new html element to be shown in the map.
  ...
  locations.forEach(function (loc) {
    ...
    .setLngLat(loc.coordinates) // Then finally add this marker HTML element inside the id="map" HTML element.
    .addTo(map); // APP POPUP
    ...
    .setLngLat(loc.coordinates) // We directly set the HTML element properties for this popup.
    .setHTML("<p>Day ".concat(loc.day, ": ").concat(loc.description, "</p>")) // Then finally add this popup HTML element inside the id="map" HTML element.
    .addTo(map); // We set the viewing boundary to include this new location point in this iteration.
      ...
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            ...
            stripe = Stripe('pk_test_51HoUFKK8GOwFHUeJoXmWJgcGPfyLitqW9gzahgjG3bg1O2TnyLKlkb91vNDHB0vsVjh2N5mCUfvUN4xC7ckt2YSC00yJOEquYU');
            _context.prev = 1;
            _context.next = 4;
...
var bookBtn = document.getElementById('book-tour'); // If we are in /tour route, then only execute these lines.
...
if ((!parent || !parent.isParcelRequire) && typeof WebSocket !== 'undefined') {
  ...
  var ws = new WebSocket(protocol + '://' + hostname + ':' + "9790" + '/');
